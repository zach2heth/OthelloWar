...
public class GWar{
  ...
  *  move options
  *  skill options
  *  adjacent objective blocks
  **********************************************/
  public void case_HasUnit(Player player, Block block){
    if(!block.getVacant() && block.getUnit().getColor() == player.getColor()){
  ...
  public void chooseOption(int posx, int posy){
    ...
    Unit nullUnit = null;
     ...
     if(gwarInter.checkInsideEndTurnBox(posx,posy)){
           ...
           gwarInter.displayPlayerBox();
           gwarInter.resetActBox();
     }
     gwarInter.checkActBox();
     resetBooleans();
  }//end of chooseOption
